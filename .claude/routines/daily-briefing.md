...
### Tier 0 深度参考 · 真实性硬规则（2026-07-11 因幻觉事故新增，最高优先级）
1. Tier 0 每条**必须来自当次真实抓取/搜索命中的结果**，且**必须附原文链接**——链接必须出现在你本次抓取输出里，禁止凭记忆拼 URL。
2. 标题用**原文标题**（可括注中文直译）；日期以**原文发布日期**为准，不许推测。
3. 抓不到实据的候选一律弃用；当期不足 3 条就如实少写，可写"本周期 Tier 0 无新增强信号"——**宁缺毋滥**。
4. **同一原文 URL 在 7 天窗口内只允许推荐一次**(跨期查重:写之前 grep 最近 7 期报告里该 URL);确要续荐须标注「同文 X 期已荐,续荐」。
5. **一条目一出处**:交叉引用他家数据必须分别署名(如 MIT 的 95% 数据不得写进 Stanford 条目),来源写谁就只能是谁——张冠李戴(如把 AINews 内容署 Latent Space)属事故。
6. **禁止把某人的"一贯观点"合成为"一篇新文章"**。事故记录：2026-07-11 曾把 Gary Marcus 六月观点脑补成"7月9日新文"（当日他并无发文）、虚构 Dwarkesh×Amodei 访谈与 Willison 文章，被逐源核验揭穿。发布前自检：每条 Tier 0 的链接能否在本次抓取记录里找到？找不到就删。


### 选题权重与解读深度(2026-07-13 从财经研判方法论移植)
- **要人直接表态优先**:AI 行业关键人物(Altman/Amodei/Hassabis/黄仁勋/苏姿丰/纳德拉/皮查伊/LeCun 及各国 AI 监管负责人)的一手表态、以及官方硬数据(财报/融资额/监管文本原文),在 Top5 遴选时优先于媒体综述,质量分上浮。
- **「这意味着」走三层递进**:事件本身 → 行业/市场即时反应 → 对企业决策的含义;只写结论不写推导的条目退回重写。
- **数据用"本期 vs 前期"给边际**:凡引用数字(融资额/用户数/算力规模),能取到前期值就并列给出,让读者看到方向而非孤立数字。
